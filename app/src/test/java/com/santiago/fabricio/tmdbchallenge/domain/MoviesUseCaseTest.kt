...
class MoviesUseCaseTest {
    ...
    private lateinit var moviesUseCase: MoviesUseCaseImpl
    ...
    fun setUp() {
        ...
        repository = mockk()
        moviesUseCase = MoviesUseCaseImpl(repository)
    }
    ...
    fun `invoke should call repository and return search results`() = runTest(testDispatcher) {
        ...
        every { repository.getMovies(any()) } returns flowOfPagingData

        val resultFlow = moviesUseCase()
